...
async def application_persistentvolumeclaim_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=application_persistentvolumeclaim_idx.__name__,
)
async def application_persistentvolumeclaim_probe(
    application_persistentvolumeclaim_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o
            for o in application_persistentvolumeclaim_idx[namespace]
        }
        for namespace in application_persistentvolumeclaim_idx
    }
...
async def application_persistentvolumeclaim(
    namespace,
    body,
    patch,
    **kwargs,
    ...
    body = (
        load(
            env.get_template("persistentvolumeclaim.yaml.j2").render(
                namespace=namespace,
                selector=label.but(
                    LABEL_CONTROLLER,
                ).asdict,
