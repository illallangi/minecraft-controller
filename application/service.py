...
async def application_service_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=application_service_idx.__name__,
)
async def application_service_probe(
    application_service_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o for o in application_service_idx[namespace]
        }
        for namespace in application_service_idx
    }
