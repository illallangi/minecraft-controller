...
@kopf.index(
    "serviceaccount",
    labels={
        LABEL_CONTROLLER: __name__,
        LABEL_NAME: CRD_SINGULAR,
    },
)
async def application_serviceaccount_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=application_serviceaccount_idx.__name__,
)
async def application_serviceaccount_probe(
    application_serviceaccount_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o for o in application_serviceaccount_idx[namespace]
        }
        for namespace in application_serviceaccount_idx
    }
...
async def application_serviceaccount(
    namespace,
    body,
    patch,
    **kwargs,
    ...
    body = (
        load(
            env.get_template("serviceaccount.yaml.j2").render(
                namespace=namespace,
                selector=label.but(
                    LABEL_CONTROLLER,
                ).asdict,
    ...
    obj = None
    try:
        obj = api.patch_namespaced_service_account(
            namespace=namespace,
