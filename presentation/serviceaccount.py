...
async def presentation_serviceaccount_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=presentation_serviceaccount_idx.__name__,
)
async def presentation_serviceaccount_probe(
    presentation_serviceaccount_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o for o in presentation_serviceaccount_idx[namespace]
        }
        for namespace in presentation_serviceaccount_idx
    }
...
async def presentation_serviceaccount(
    namespace,
    body,
    patch,
    **kwargs,
