...
async def presentation_deployment_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=presentation_deployment_idx.__name__,
)
async def presentation_deployment_probe(
    presentation_deployment_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o for o in presentation_deployment_idx[namespace]
        }
        for namespace in presentation_deployment_idx
    }
...
async def presentation_deployment(
    namespace,
    body,
    patch,
    **kwargs,
    ...
    for kind in [
        ("presentation", "configmap"),
        ("presentation", "serviceaccount"),
        ("tls", "certificate"),
    ]:
        try:
