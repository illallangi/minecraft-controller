...
@kopf.index(
    ...
    version="v1",
    singular="dnsresourcerecord",
    labels={
        LABEL_CONTROLLER: __name__,
        LABEL_NAME: CRD_SINGULAR,
    },
)
async def presentation_dnsresourcerecord_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=presentation_dnsresourcerecord_idx.__name__,
)
async def presentation_dnsresourcerecord_probe(
    presentation_dnsresourcerecord_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o
            for o in presentation_dnsresourcerecord_idx[namespace]
        }
        for namespace in presentation_dnsresourcerecord_idx
    }
...
async def presentation_dnsresourcerecord(
    namespace,
    body,
    patch,
    **kwargs,
    ...
    body = (
        load(
            env.get_template("dnsresourcerecord.yaml.j2").render(
                namespace=namespace,
                selector=label.but(
                    LABEL_CONTROLLER,
                    LABEL_INSTANCE,
