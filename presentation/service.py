...
async def presentation_service_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=presentation_service_idx.__name__,
)
async def presentation_service_probe(
    presentation_service_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o for o in presentation_service_idx[namespace]
        }
        for namespace in presentation_service_idx
    }
...
async def presentation_service(
    namespace,
    body,
    patch,
    **kwargs,
    ...
    body = (
        load(
            env.get_template("service.yaml.j2").render(
                namespace=namespace,
                selector=label.but(
                    LABEL_CONTROLLER,
                    LABEL_INSTANCE,
