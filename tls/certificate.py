...
@kopf.on.probe(
    id=tls_certificate_idx.__name__,
)
async def tls_certificate_probe(
    tls_certificate_idx: kopf.Index,
    **_,
):
    return {
        namespace: {o["metadata"]["name"]: o for o in tls_certificate_idx[namespace]}
        for namespace in tls_certificate_idx
    }
...
async def tls_certificate(
    ...
    **kwargs,
    ...
    body = (
        load(
            env.get_template("certificate.yaml.j2").render(
                namespace=namespace,
                selector=label.but(
                    LABEL_CONTROLLER,
                    LABEL_INSTANCE,
